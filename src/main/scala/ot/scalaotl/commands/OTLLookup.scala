...
class OTLLookup(sq: SimpleQuery) extends OTLBaseCommand(sq, _seps = Set("output", "outputnew")) with OTLLookups with ReplaceParser with OTLSparkSession {
  ...
  val lookupFile = args.split(" ").headOption.map(_getLookupPath)
  val inputs = args.split(seps.map(_.addExtraSpaces).mkString("|")).toList match {
    case head :: tail => returnsParser(head.split(" ").drop(1).mkString(" "), Set.empty)
    case _            => Return(List[ReturnField]())
  }

  val _list = args.split(seps.map(_.addExtraSpaces).mkString("|")).toList
  override val returns = _list match {
  ...
  override def transform(_df: DataFrame): DataFrame = {
    ...
    if (initInputCols.nonEmpty
      ...
      val dfJoined = _df.join(jdfSelect, initInputCols.toSeq, "left")
