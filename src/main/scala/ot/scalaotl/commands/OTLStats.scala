...
class OTLStats(sq: SimpleQuery) extends OTLBaseCommand(sq, _seps = Set("by")) with StatsParser with WildcardParser {
  ...
  val timeColumn: String = getKeyword("timeCol").getOrElse("_time")
  ...
  val joinedColumns = positionalsMap.get("by") match {
  ...
  override def transform(_df: DataFrame): DataFrame = {
    ...
    val startChecking = if (timeFuncs.contains(aggFuncs.head.func)) {timeChecking} else {nonTimeChecking}
    ...
    val firstFuncsList: List[List[StatsFunc]] = if (startChecking == timeChecking) {timeFuncsList} else nonTimeFuncsList
    val secondFuncsList: List[List[StatsFunc]] = if (firstFuncsList == timeFuncsList) {nonTimeFuncsList} else timeFuncsList
 ...
 *
  ...
  private def calcTimeFuncs(df: DataFrame, func: StatsFunc): DataFrame = {
    ...
    val dfWithTimeMax = dfIndexed.withColumn("__bndrVal__",
      (if (isFirst){min(timeColumn)} else {max(timeColumn)}).over(window).alias("__bndr__"))
