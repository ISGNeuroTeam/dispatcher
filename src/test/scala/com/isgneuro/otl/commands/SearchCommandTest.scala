...
import com.isgneuro.scalaotl.testutils.DataFrameTestUtils.compareDataFrames
import org.apache.spark.sql.types.{LongType, StringType, StructField, StructType}
import org.apache.spark.sql.{Row, functions => F}

class SearchCommandTest extends Command2Test {
  test("SEARCH => TEST 1. Simple read only _time and _raw fields") {
    val otlQuery = createQuery("", "search", s"$test_index-0")
    val actual = executeFull(otlQuery)
  ...
  test("SEARCH => TEST 2. Apply time range") {
    val otlQuery = createQuery("", "search", s"$test_index-0", startTime, finishTime)
    val actual = executeFull(otlQuery)
    val expected = readIndexDF(s"$test_index-0", readingDatasetSchema)
      .select(F.col("_time"), F.col("_raw"))
      .filter(F.col("_time").between(startTime, finishTime))
    compareDataFrames(actual, expected)
  }

  ignore("SEARCH => TEST 3. Add empty cols") {
  ...
  test("SEARCH => TEST 4. Reading of all indexes") {
    val otlQuery = createQuery("", "search", "*")
    var actual = executeFull(otlQuery)
  ...
  test("SEARCH => TEST 5. Reading indexes by name with wildcard") {
    val otlQuery = createQuery("", "search", "$test_index*")
    var actual = executeFull(otlQuery)
    var expected = spark.createDataFrame(spark.sparkContext.emptyRDD[Row],
      StructType(Seq(StructField("_time", LongType), StructField("_raw", StringType))))
    for (i <- stfeRawSeparators.indices) {
  ...
  test("SEARCH => TEST 6. Read indexes with wildcard. One of indexes is not exist ") {
  ...
  ignore("READ => TEST 7. Read indexes with wildcard. All indexes is not exist ") {
    /*val non_existent_index = "non_existent_index"
  ...
  ignore("READ => TEST 8. Read fields from index with with field name wildcards") {
  ...
  ignore("READ => TEST 9.Read index with filter on existing fields") {
    /*val query = createFullQuery(
  ...
  ignore("READ => TEST 10.Read index with filter on not existing fields") {
    /*val query = createFullQuery(
  ...
  ignore("READ => TEST 11. Read index with filter on not existing field with negation") {
    /*val query = createFullQuery(
  ...
  ignore("READ => TEST 12. Read index with filter on not existing field with negation and {}") {
    /*val query = createFullQuery(
  ...
  ignore("READ => TEST 13. Read index with filter on existing field with negation and {}") {
    /*val query = createFullQuery(
  ...
  ignore("READ => TEST 14. Read index with filter on existing field with negation and {} using ()") {
    /*val query = createFullQuery(
      s"""search index=$test_index-0 (text=RUB 'text{2}.val'!=null) | table _time, text, 'text{1}.val', 'text{2}.val'""",
      s""" | read {"$test_index-0": {"query": "(text=\\\"RUB\\\" AND !('text{2}.val'=\\\"null\\\"))", "tws": 0, "twf": 0}} | table _time, text, 'text{1}.val', 'text{2}.val'""",
      s"$test_index-0")
  ...
  ignore("READ => TEST 15. Create multi-value cols") {
  ...
  ignore("READ => TEST 16. Create multi-value cols") {
  ...
  ignore("READ => TEST 17. Replace square brackets with curly brackets in col names") {
