...
import com.isgneuro.scalaotl.exceptions.builders.EvalExceptionInvoker
  ...
  override def getFieldsUsed: Return => List[String] = (ret: Return) => {
    ret.evals.flatMap {
      ...
      case x => try {
        ...
        case e: Exception =>
          EvalExceptionInvoker.doThrow(x.newfield, e.getMessage)
      }
    }
  }
  ...
  override def validateArgs(): Unit = {
    try {
      ...
      case e: CustomException =>
        EvalExceptionInvoker.doThrow("No fields", e.getMessage)
    }
  }
