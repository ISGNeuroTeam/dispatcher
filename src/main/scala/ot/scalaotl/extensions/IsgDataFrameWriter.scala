...
import org.apache.spark.sql.{AnalysisException, DataFrameWriter, Dataset, SaveMode, SparkSession}
...
class IsgDataFrameWriter[T](ds:Dataset[T]) {
   ...
   * @since 1.4.0
   */
  def mode(saveMode: SaveMode): IsgDataFrameWriter[T] = {
    this.mode = saveMode
    this
  }
   ...
   * @since 1.4.0
   */
  def mode(saveMode: String): IsgDataFrameWriter[T] = {
   ...
   * @since 1.4.0
   */
  def format(source: String): IsgDataFrameWriter[T] = {
    this.source = source
    this
  }
   ...
   * @since 1.4.0
   */
  def option(key: String, value: String): IsgDataFrameWriter[T] = {
   ...
   * @since 2.0.0
   */
  def option(key: String, value: Boolean): IsgDataFrameWriter[T] = option(key, value.toString)
   ...
   * @since 2.0.0
   */
  def option(key: String, value: Long): IsgDataFrameWriter[T] = option(key, value.toString)
   ...
   * @since 2.0.0
   */
  def option(key: String, value: Double): IsgDataFrameWriter[T] = option(key, value.toString)
   ...
   * @since 1.4.0
   */
  def options(options: scala.collection.Map[String, String]): IsgDataFrameWriter[T] = {
   ...
   * @since 1.4.0
   */
  def options(options: java.util.Map[String, String]): IsgDataFrameWriter[T] = {
    ...
    if (classOf[DataSourceV2].isAssignableFrom(cls)) {
      ...
      source match {
        case ws: WriteSupport =>
          val sessionOptions = IsgDataSourceV2Utils.extractSessionConfigs(
            source,
  ...
  private var sortColumnNames: Option[Seq[String]] = Nonetils.extractSessionConfigs(
            source,
            df.sparkSession.sessionState.conf)
          val options = sessionOptions.filterKeys(!extraOptions.contains(_)) ++ extraOptions.toMap
